...
>
	...
	<p class="mb-3 text-base font-medium text-surface-900">{question}</p>

	<pre class="mb-4 overflow-x-auto rounded-lg bg-surface-900 p-4 text-sm text-surface-100"><code>{code}</code></pre>
	...
	{:else}
		...
		>
			<p class="mb-1 font-semibold">
				...
				{#if !isCorrect}
					The expected answer was: <code class="rounded bg-surface-200 px-1.5 py-0.5 text-xs font-mono">{correct}</code>
				{/if}
			</p>
			<p>{explanation}</p>
		</div>
